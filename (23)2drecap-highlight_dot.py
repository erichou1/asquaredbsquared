...
class TetrahedronZoom(ThreeDScene):
    def construct(self):
        ...
        for i, vertex in enumerate(vertex_coords):
            ...
            label = Text(labels[i], weight=BOLD, slant=ITALIC).scale(0.5).move_to(
                label_position + np.array([0, 0, 0.2])
            )
        ...
        # --- Step 4: Highlight faces sequentially ---
        # --- Step 4: Highlight faces sequentially ---
        face_sets = [
            [0, 1, 2],  # face 1 vertices
            [0, 1, 3],  # face 2 vertices
            [0, 2, 3],  # face 3 vertices
        ]

        for vertex_indices in face_sets:
            # Highlight vertices
            vertex_mobs = []
            for i in vertex_indices:
                dot = Dot3D(vertex_coords[i], color=GOLD, radius=0.1)
                dot.z_index = 10  # render above the tetrahedron
                vertex_mobs.append(dot)

            # Highlight face
            face_polygon = Polygon(
                *[vertex_coords[i] for i in vertex_indices],
                color=GOLD,
                fill_color=GOLD,
                fill_opacity=0.6,
                shade_in_3d=True
            )
            face_polygon.z_index = 5  # above tetrahedron faces

            # Animate vertices + face at the same time
            self.play(
                AnimationGroup(
                    *[Create(dot) for dot in vertex_mobs],
                    FadeIn(face_polygon),
                    run_time=0.75,
                    lag_ratio=0
                )
            ...
            # Immediately fade out, no wait in between
            self.play(FadeOut(face_polygon), FadeOut(*vertex_mobs))
