...
class ThreeDTriangle(ThreeDScene):
    def construct(self):
        ...
        self.camera.frame_center = ([(0.0, -1.1, 0)])
        # Triangle vertices
        ...
        def create_ticks(p1, p2, color=WHITE, tick_len=0.4, spacing=0.07):
            ...
            pos1 = mid + offset
            pos2 = mid - offset
            tick1 = Line(pos1 - perp / 2, pos1 + perp / 2, color=color, stroke_width=5)
            tick2 = Line(pos2 - perp / 2, pos2 + perp / 2, color=color, stroke_width=5)
        ...
        traced_BN.animate.put_start_and_end_on(v2, moving_dot2.get_center())
        traced_DN.animate.put_start_and_end_on(extended_point, moving_dot2.get_center())
        ...
        extended_perp_line.z_index = 999
        self.play(
            extended_perp_line_C.animate.set_color(BLUE), 
            point_d.animate.set_stroke(opacity=0),
            point_d2.animate.set_stroke(opacity=1),
            point_e2.animate.set_stroke(opacity=1),
            point_e.animate.set_stroke(opacity=0),
            run_time=2
        )
        self.remove(point_d,point_e, CE)
                  
        self.wait()
        self.move_camera(zoom=0.45, run_time=0.5)
        self.wait()
        self.play(label_d.animate.move_to([-6.54404159,  1.85232812,  0.        ]),
            label_e.animate.move_to([ 0.0,         -9.36349888,  0.0        ]))
        self.move_camera(phi=60 * DEGREES, theta=29 * DEGREES, frame_center=([(-2.25, -3, 0)]), run_time=5)
        ...
        label_d2.add_updater(lambda mob: mob.next_to(temp_bd_group[1], DOWN, buff=0.35))
        ...
        def bd_update(mob):
            alpha = bd_tracker.get_value()
            rotated = bd_group2.copy().rotate(alpha * angle_bd, axis=axis_ac, about_point=v1)
            mob.become(rotated)

            mob.set_stroke(opacity=1).set_fill(opacity=1)
            for i, outline in enumerate(bd_outlines):
                if alpha >= (i + 1) / num_steps:
                    for submob in outline:
                        submob.set_opacity(0.15)

        temp_bd_group.add_updater(bd_update)
        ...
        self.play(
            bd_tracker.animate.set_value(1), 
            labels[0].animate.move_to([-5.8, -4.4,  0.        ]),
            labels[1].animate.move_to([[ 3.55122106, -3.31799997,  0.3        ]]),
            labels[2].animate.move_to([0.,         3.11973773, 0.3        ]),
            run_time=2.9, rate_func=smooth)
        self.wait()
        extended_perp_line.set_stroke(opacity=1)
        point_d2.set_fill(opacity=1)

        temp_bd_group.remove_updater(bd_update)
        ...
        label_e2.add_updater(lambda mob: mob.next_to(temp_ce_group[1], DOWN, buff=0.35))
        # ========== CE ROTATION ==========
        ...
        def ce_update(mob):
            alpha = ce_tracker.get_value()
            rotated = ce_group2.copy().rotate(alpha * angle_ce, axis=axis_ab, about_point=v1)
            mob.become(rotated)
            mob.set_stroke(opacity=1).set_fill(opacity=1)

            for i, outline in enumerate(ce_outlines):
                if alpha >= (i + 1) / num_steps:
                    for submob in outline:
                        submob.set_opacity(0.15)
        temp_ce_group.add_updater(ce_update)
        ...
        self.play(ce_tracker.animate.set_value(1), run_time=3, rate_func=smooth)
        self.wait()
        extended_perp_line_C.set_stroke(opacity=1)
        point_e2.set_fill(opacity=1)
        temp_ce_group.remove_updater(ce_update)
        ...
        # self.add(ce_group)
        print(f"{label_d2.get_center()}", f"{label_e2.get_center()}", )
        self.wait(2)
